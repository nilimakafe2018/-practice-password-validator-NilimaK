...
while (!isValid) {
  Mypassword = readlineSync.question("Please enter your password,:");

//Checking the length of the password.
 if (Mypassword.length < 8) { // checking if the length is less than 8 or not 
 ...
 for (let i = 0; i < Mypassword.length; i++) {
   if (Mypassword[i] >= 'A' && Mypassword[i] <= 'Z') { //checking if the password has uppercase A-Z or not
     hasUppercase = true;
     break;
   }
 }
 ...
 let hasNumber = false;
 for (let i = 0; i < Mypassword.length; i++) {
   if (Mypassword[i] >= '0' && Mypasswordclear[i] <= '9') { //checking if the number is between 0 to 9 or not
     hasNumber = true;
     break;
   }
 }
